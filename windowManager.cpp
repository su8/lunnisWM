...
void WindowManager::handleConfigureRequest(XConfigureRequestEvent *ev)
{
    ...
    if (frameHandlesTOFrame.count(ev->parent)) //If parent is a frame
    {
        ...
        frameHandlesTOFrame[ev->parent].moveResizeFrame(ev->x, ev->y, ev->width, (ev->height + 25));
    }
}
...
void WindowManager::handleButtonPress(XButtonEvent *ev)
{
    ...
    case Button1:
        if (ev->state & Mod1Mask) // If alt button is being pressed get window and curs info for window transformations
        {
            ...
            XGetGeometry(display, ev->window, &root_return, &winIntX, &winIntY, &winIntWidth, &winIntHeight, &border_width_return, &depth_return);
            //Get curs pos
            cursIntX = ev->x_root;
            cursIntY = ev->y_root;
        }
        ...
        else
        {
            ...
            if (frameHandlesTOFrame.count(parent))
            {
                ...
                unsigned int depth_return;
                //Get location and size of frame
                XGetGeometry(display, parent, &root_return, &winIntX, &winIntY, &winIntWidth, &winIntHeight, &border_width_return, &depth_return);
                //Get curs pos
                cursIntX = ev->x_root;
                cursIntY = ev->y_root;
            }
...
void WindowManager::handleMotionNotify(XMotionEvent *ev)
{
    ...
    case Button1Mod1Mask: // Case alt and left mouse button are/have been pressed
        frameHandlesTOFrame[ev->window].handleWindowDrag(ev, cursIntX, cursIntY, winIntX, winIntY);
        break;
    case Button1MotionMask:
        frameHandlesTOFrame[active].handleWindowDragOrResize(ev, cursIntX, cursIntY, winIntX, winIntY, winIntWidth, winIntHeight);
        break;
